...
class Template
{
     ...
     * @return string
     */
    public function render(array $matriz = array(), array $urls = array())
    {
        ...
        foreach($lines as $line) {
            ...
            if (preg_match("/(\{[^\}]+\}\})/i", $line, $areaComandoLlaves) > 0){
                ...
                if(count(preg_grep("/{{([a-z]\w+?)}}/", explode("\n", $line))) > 0){
                    ...
                    $command = new Command($areaComandoLlaves, $line, $this->variables);
                }
...
{   
    ...
    */
     ...
     * @param string $filename
     */
    public function __construct($areaComandoLlaves, $line, $variables)
    {
    ...
    public function setComando($line){
        if(preg_match("/{{\s*(\w+)\s+([\w\"\.\/\-\_\?]+)\s*}}/i", $this->areaComandoLlaves[1], $grupos) > 0){
            ...
            switch ($funcion) {
                case 'embed':
                    $this->patron = new Function_Embed($variable, $this->variables);
                    echo $this->patron->render();
                    break;

                case 'include':
                    // var_dump("include " .$funcion ." ". $variable);
                    $this->patron = new Function_Include($variable);
                    echo $this->patron->render();
                    break;
                case 'for':
                    var_dump("for " .$funcion ." ". $variable);
                    $this->patron = new Function_For($variable, "aaaa");
                    echo $this->patron->render();
                    break;
                default:
                    # code...
...
class Function_Embed extends Command_Function
{
    ...
    public function __construct($filename, $variables)
    {
        ...
        if($url = $this->chequeoRuta($filename)){
            ...
            $url = $this->chequeoRuta($variables[$filename]);
            $this->_filename = $url;
        }
    }
...
class Function_For
{
    ...
    public function __construct($variables, $html)
    {
        // var_dump($filename);
